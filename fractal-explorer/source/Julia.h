...
namespace Fractal
{
	...
	public:
		...
		Julia(unsigned int vpWidth, unsigned int vpHeight, unsigned short max_iterations, D3DCOLOR* pRenderTarget, Palette palette, bool autoAnimate = false) : Fractal(vpWidth, vpHeight, max_iterations, pRenderTarget, autoAnimate)
		{
			...
			normalizer.ox = normalizer.dx * 0.5000000000000000;
		...
		void DrawFractal(unsigned char nThreads) override
		{
			...
			for (unsigned char t = 0; t < nThreads; t++)
				...
				{
					Complex Z(normalizeX(pxLUT[pixel].x), normalizeY(pxLUT[pixel].y));
